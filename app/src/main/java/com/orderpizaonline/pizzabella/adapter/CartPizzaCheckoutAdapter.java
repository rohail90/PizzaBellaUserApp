...
public class CartPizzaCheckoutAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {

    private List<PizzaOrderCartModel> mList = new ArrayList<>();
    private Context context;

    public CartPizzaCheckoutAdapter(Context context, List<PizzaOrderCartModel> mList) {
        this.mList = mList;
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        return new CartPizzaCheckoutHolder(view , mList, context);
    }

    @Override
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        int size= mList.size();
        ((CartPizzaCheckoutHolder)holder).bindData(mList.get(position),position,size);
