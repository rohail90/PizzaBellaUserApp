...
public class CartMealAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {

    private List<ComboOrderInfo> mList = new ArrayList<>();
    private Context context;

    public CartMealAdapter(Context context, List<ComboOrderInfo> mList) {
        this.mList = mList;
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        return new CartMealHolder(view , mList, context);
    }

    @Override
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        int size= mList.size();
        ((CartMealHolder)holder).bindData(mList.get(position),position,size);
