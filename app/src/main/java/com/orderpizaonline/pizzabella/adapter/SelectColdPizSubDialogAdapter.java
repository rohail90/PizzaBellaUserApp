...
public class SelectColdPizSubDialogAdapter extends RecyclerView.Adapter<SelectColdPizSubDialogAdapter.ViewHolder> {

    private List<PairValuesModel> mList = new ArrayList<>();
    private Context mContext;
    private int index = 0;

    public SelectColdPizSubDialogAdapter(List<PairValuesModel> mList, Context context) {
        this.mList = mList;
    ...
    @NonNull
    @Override
    public SelectColdPizSubDialogAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {

        View view = LayoutInflater.from(mContext).inflate(R.layout.sub_dialog_item, parent, false);
        return new SelectColdPizSubDialogAdapter.ViewHolder(view);
    }


    @Override
    public void onBindViewHolder(@NonNull SelectColdPizSubDialogAdapter.ViewHolder holder, final int position) {
        ...
        holder.cl_sub_item.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                selectColdPizItm.setName(mList.get(position).getName());
            }
        });

        holder.name.setText(mList.get(position).getName());
        holder.price.setText("" + mList.get(position).getValue());
