...
import static com.orderpizaonline.pizzabella.fragments.CartCheckoutFragment.orderPizzas;
import static com.orderpizaonline.pizzabella.fragments.CartCheckoutFragment.setTotalCartPrice;


public class CartPizzaCheckoutHolder extends RecyclerView.ViewHolder {
    ...
    public CartPizzaCheckoutHolder(@NonNull View itemView, List<PizzaOrderCartModel> mList, Context context) {
        super(itemView);
...
//        lv_extras = itemView.findViewById(R.id.lv_extras);
    ...
    public void bindData(final PizzaOrderCartModel achievementModel, final int position, int size) {
        ...
        /*List<SidelineOrderInfo> pizzaExtraToppingList = new ArrayList<>(achievementModel.getPizzaExtraToppingList());
        int totPrice = 0;
        totPrice = achievementModel.getQuantity() * Integer.parseInt(achievementModel.getPrice());
        for (int i = 0; i < imgUriList.size(); i++) {
            totPrice +=  Integer.parseInt(imgUriList.get(i).getPrice());
        }
        ...
        if (achievementModel.getComboInfo() != null){
            totPrice += Integer.parseInt(achievementModel.getComboInfo().getPrice());
        }
        if (achievementModel.getSauce() != null) {
            if (achievementModel.getSauce().getPrice() != null) {
                totPrice += Integer.parseInt(achievementModel.getSauce().getPrice());
            }
        }*/

     /*   if (achievementModel.getPizzaType().equals(Common.PIZZA_TYPE_FULL_CIRCLE)) {
            rl_full_circle.setVisibility(View.VISIBLE);
            rl_slice.setVisibility(View.GONE);
            rl_semi_circle.setVisibility(View.GONE);
            rl_full_square.setVisibility(View.GONE);
...
//                        Picasso.get().load(achievementModel.getComboInfo().getImageURL()).into(destination);
            ...
            rl_full_circle.setVisibility(View.GONE);
            rl_semi_circle.setVisibility(View.GONE);
            rl_full_square.setVisibility(View.GONE);
...
/*
        ...
        if (imgUriList.size() > 0) {
            if (imgUriList.get(0).getItemName() != null && !imgUriList.get(0).getItemName().equals("")) {
                tv_toppings_name.setText(imgUriList.get(0).getItemName());
            }
            String vegeis = "";
            for (int i = 0; i < imgUriList.get(0).getVegeInfoArrayList().size(); i++) {
...
//                            Picasso.get().load(achievementModel.getComboInfo().getImageURL()).into(square_destination);
        ...
        if (achievementModel.getPizzaExtraToppingList() != null) {
            if (achievementModel.getPizzaExtraToppingList() .size() > 0) {
                tv_extras_name.setText(achievementModel.getExtraTopping().getName());
                tv_extras_price.setText(achievementModel.getExtraTopping().getValue());
       ...
       /* }*/
        ...
        tv_pizza_price.setText(Integer.parseInt(achievementModel.getPrice())*achievementModel.getQuantity() + "");
        ...
        plus.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                int pr= Integer.parseInt(mList.get(position).getSizePrice());
